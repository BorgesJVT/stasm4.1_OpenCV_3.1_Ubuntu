...
int borges_stasm_init(            // call once, at bootup (to read models from disk)
    string datadir,   // in: directory of face detector files
    int         trace)     // in: 0 normal use, 1 trace to stdout and stasm.log
{
    const char * c = datadir.c_str();
    return stasm_init_ext(c, trace, NULL);
}
